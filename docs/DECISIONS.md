...
- **Design Parity**: Re-engineered to achieve 1-to-1 visual and physical parity with Obsidian Design's floating dock.
...
- **Icon Library & Order**:
  ...
  - Aligned exact icon order matching the obsidian layout:
...
### Why was it used by obsidian in the original site?
...
| **Option B: Pure Curated Crates Showcase** | Keep Spotify on Home, but format it purely as curated crates with direct Spotify launch links, removing the expectation that it serves as an in-page audio player. | Retains obsidian visual parity. Zero code changes required. | Still unmounts if played; 30-sec limit applies for unauthenticated guests. |
